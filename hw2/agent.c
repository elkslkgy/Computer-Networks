...
int main(int argc, char* argv[]){
    ...
    if(argc != 8){
        fprintf(stderr,"用法: %s <sender IP> <agent IP> <recv IP> <sender port> <agent port> <recv port> <loss_rate>\n", argv[0]);
        fprintf(stderr, "例如: ./agent local local local 8887 8888 8889 0.3\n");
        exit(1);
    } else {
        setIP(ip[0], argv[1]);  //sender IP
        setIP(ip[1], argv[2]);  //agent IP
        setIP(ip[2], argv[3]);  //recv IP

        sscanf(argv[4], "%d", &port[0]);    //sender port
        sscanf(argv[5], "%d", &port[1]);    //agent port
        sscanf(argv[6], "%d", &port[2]);    //recv port

        sscanf(argv[7], "%f", &loss_rate);
    }

    /*Create UDP socket*/
    if ((agentsocket = socket(PF_INET, SOCK_DGRAM, 0)) == -1) {
    ...
    /*bind socket*/
    if ((bind(agentsocket, (struct sockaddr *)&agent, sizeof(agent))) == -1) {
        close(agentsocket);
    ...
    receiver.sin_port = htons(port[2]); //recv port
    receiver.sin_addr.s_addr = inet_addr(ip[2]);    //recv IP
